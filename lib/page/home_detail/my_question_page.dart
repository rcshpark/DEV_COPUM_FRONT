...
class _MyQuestionPageState extends State<MyQuestionPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final userP = Provider.of<UserInfoProvider>(context);
              ...
              child: p.questionModel.message == "질문 조회 성공"
                  ? ListView.separated(
                      itemBuilder: (context, index) {
                        return p.questionModel.result![index].creator ==
                                userP.userModel.id.toString()
                            ? Column(
                                children: [
                                  InkWell(
                                    onTap: () async {
                                      // 클릭한 질문에 대한 답변글들 조회 -> page 이동
                                      await answerP.fetchData(p.questionModel
